...
SetUsage @ "
CharacterData[char$, prop$] gives the value of the specified ICU/Unicode property prop$ for the \
character specified by char$.
CharacterData[char$, prop$, subprop$] gives the subproperty subprop$ for a given prop$.
";
...
expr : CharacterData[args__] :=
  With[{res = Catch[iCharacterData[HoldForm @ expr, args], $$tag]},
    res /; res =!= $$unevaluated
  ];
...
CharacterData::errchar =
  "The argument at position 1 in `1` should either be an integer between 0 and 1114111, \
a character (string of length 1) or a list of these values.";
CharacterData::errprop =
  "The argument at position 2 in `1` is not a member of CharacterData[All, \"Properties\"].";
CharacterData::errsprop =
  "The argument at position 3 in `1` is neither Automatic, \"Date\", \"Name\" nor \"ShortName\".";
